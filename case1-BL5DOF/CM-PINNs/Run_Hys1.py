...
from basecode.Hysteresis_MDOF_NewMark import Hysteresis_MDOF_NewMark1
...
class Dataset:
    def __init__(self, datafile, slice_start, slice_end, transform=None, target_transform=None):
        ...
        self.Y_u = torch.as_tensor(self.mat['target_u'][slice_start:slice_end, :, 2:])
        self.Y_v = torch.as_tensor(self.mat['target_v'][slice_start:slice_end, :, 2:])
        self.Y_a = torch.as_tensor(self.mat['target_a'][slice_start:slice_end, :, 2:])
        self.Y_ar = torch.as_tensor(self.mat['target_ar'][slice_start:slice_end, :, 2:])
...
def Test_in_train_Processing(Config, model_test):
    ...
    '''获取训练集和测试集中加速度的平均峰值误差（以字符百分比形式输出）'''
...
class ModelConfig:
    def __init__(self, data_name):
        ...
        self.hys = Hysteresis_MDOF_NewMark1(m=1e3 * torch.tensor([180, 180, 180]),
                                            k0=1e6 * torch.tensor([98, 98, 98]),
                                            fy=1e3 * torch.tensor([490, 490, 490]),
                                            alpha=torch.tensor([0.1, 0.1, 0.1]),

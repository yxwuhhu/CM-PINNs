...
class HysU_RS:
    """
    Used to input the displacement response to obtain
    the restoring force response of a bilinear intrinsic
    single-degree-of-freedom system system
    ...
    def state_determine_nsdof(self, k1, last_r, last_u, u):
        ...
        fyl_minus_b = torch.full_like(k1, fyl_minus_b)  # Extend the dimension of the tensor to match the dimension of k1
        k2 = self.alpha * k1
        d_u = u - last_u
        ...
        c3 = 1.0 * fyl_minus_b
        c = last_r + k1 * d_u
        R = torch.max(c1 - c2, torch.min(c1 + c3, c))  # Vectorization using torch.max and torch.min
...
class Loss_PhyHysLSTM(nn.Module):
    ...
    def forward(self, ag_all, pred_data_all, true_data):
        ...
        if self.is_run1:
            ...
            PhyLoss = (PhyLoss_v + PhyLoss_e + PhyLoss_r + PhyLoss_ar)/4

...
class LSTMModel1(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.N2FC1_ag = nn.Sequential(nn.Linear(1, self.n_hidden, bias=True), nn.ReLU())
    ...
    def forward(self, x):
        ...
        N2FC1_ag = self.N2FC1_ag(x)

        # Net2
        N2L1_out, _ = self.N2L1(torch.cat([N2FC1_ag, out], dim=2))
...
class LSTMModel2(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.lstm2 = nn.LSTM(n_hidden, n_hidden, batch_first=True)
        self.activation2 = nn.ReLU()
        self.N2FC1_uvr = nn.Sequential(nn.Linear(3, self.n_hidden, bias=True), nn.ReLU())
        self.N2L1 = nn.LSTM(2 * self.n_hidden, self.n_hidden, batch_first=True)
        self.N2FC3 = nn.Linear(self.n_hidden, self.n_hidden, bias=True)
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm2(out)
        out = self.activation2(out)
        N2FC1_uvr = self.N2FC1_uvr(x)

        # Net2
        N2L1_out, _ = self.N2L1(torch.cat([N2FC1_uvr, out], dim=2))
        N2FC3_out = F.relu(self.N2FC3(N2L1_out))
...
class LSTMModel3(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.lstm2 = nn.LSTM(n_hidden, n_hidden, batch_first=True)
        self.activation2 = nn.ReLU()
        self.N2FC1_vr = nn.Sequential(nn.Linear(2, self.n_hidden, bias=True), nn.ReLU())
        self.N2L1 = nn.LSTM(2 * self.n_hidden, self.n_hidden, batch_first=True)
        self.N2FC3 = nn.Linear(self.n_hidden, self.n_hidden, bias=True)
        self.N2_OUT = nn.Linear(self.n_hidden, 1)
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm2(out)
        out = self.activation2(out)
        N2FC1_vr = self.N2FC1_vr(x)

        # Net2
        N2L1_out, _ = self.N2L1(torch.cat([N2FC1_vr, out], dim=2))
        N2FC3_out = F.relu(self.N2FC3(N2L1_out))
...
class PhyLSTM3(nn.Module):
    ...
    def forward(self, ag, model_type=''):
        ...
        if model_type == 'data':
            output = torch.cat((u, v, vdot, ar), dim=-1)
        elif model_type == 'phy':
            output = torch.cat((u, udot, v, vdot, ar, rdot1, rdot2), dim=-1)
        else:
            output = torch.cat((u, v, vdot, ar), dim=-1)
        return output
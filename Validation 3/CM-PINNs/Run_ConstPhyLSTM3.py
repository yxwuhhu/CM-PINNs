...
class Dataset:
    def __init__(self, datafile, slice_start, slice_end, is_run1=True, transform=None, target_transform=None):
        ...
        self.Y_ar = torch.as_tensor(self.mat['target_ar'][slice_start:slice_end])
        self.Xc = torch.as_tensor(self.mat['input'][0:50]).float().cuda()
        if is_run1:
            self.Y = torch.stack([self.Y_u, self.Y_v, self.Y_a, self.Y_ar], dim=2)
        else:
            self.Y = self.Y_u
        self.process_data()
...
def Test_in_train_Processing(Config, model_test):
    ...
    test_pred, test_target = test_pred, test_target
...
class ModelConfig:
    def __init__(self, data_name):
        ...
        self.model_type1 = 'proposed_model_6'
        ...
        self.loss_fn1 = Loss_PhyHysLSTM(Dif=self.finite_difference)
    ...
    def get_data_loader(self, is_run1=True, need_val=True):
        def create_data_slice(start, end, batch_size, need_shuffle=True):
            data = Dataset(datafile=self.data_path + self.data_name + ".mat", transform=self.get_transform(),
                           slice_start=start, slice_end=end, is_run1=is_run1, target_transform=self.get_transform())

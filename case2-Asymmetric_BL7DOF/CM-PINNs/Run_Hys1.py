...
from basecode.Hysteresis_AsymMDOF_NewMark import Hysteresis_Asym_MDOF_NewMark
...
class Dataset:
    def __init__(self, datafile, slice_start, slice_end, transform=None, target_transform=None):
        ...
        self.Y_u = torch.as_tensor(self.mat['target_u'][slice_start:slice_end, :, 2:5])
        self.Y_v = torch.as_tensor(self.mat['target_v'][slice_start:slice_end, :, 2:5])
        self.Y_a = torch.as_tensor(self.mat['target_a'][slice_start:slice_end, :, 2:5])
        self.Y_ar = torch.as_tensor(self.mat['target_ar'][slice_start:slice_end, :, 2:5])
        self.Xc = torch.as_tensor(self.mat['input'][0:50]).float().cuda()
        self.Y = torch.cat([self.Y_u, self.Y_v, self.Y_a, self.Y_ar], dim=-1)
        self.process_data()
...
def train_one_epoch1(scaler, Config, epoch):
    ...
    for batch, (ag_train, target_train) in enumerate(Config.trainLoader1):
        ...
        batch_train_loss = Config.loss_fn1((batch_ag_train, batch_ag_c_train),
                                           ...
                                           batch_target_train,
                                           is_start=True if epoch == 0 else False)
...
def train_loop1(Config):
    ...
    for epoch in range(Config.max_epoch1):
        ...
        train_loss = train_one_epoch1(scaler, Config, epoch)
        Train_loss1.append(train_loss)
        Val_loss1.append(train_loss)
        ...
        if epoch % 1 == 0:
            ...
            print('Epoch: %d, Train loss: %.5e, Val loss: %.5e, Time: %.2f秒'
                  % (epoch, train_loss, train_loss, time_length))

    time_counter(start_time_all, time.time())
    torch.save(
        Config.model_train1,
        Config.model_path1 + 'FModel.pth',
        pickle_protocol=3)  # save model
    draw_loss(
...
def Test_in_train_Processing(Config, model_test):
    ...
    return MSE_t, PeakError_max


def Test_And_Save_Processing(Config):
    model_test = torch.load(Config.testmodel_path1)
    test_input, test_pred, test_target = test_loop1(Config=Config, model_test=model_test)
    # test_pred, test_target = test_pred[:, :, 0 * 3:1 * 3], test_target[:, :, 0 * 3:1 * 3]
    # test_pred, test_target = test_pred[:, :, 1*3:2*3], test_target[:, :, 1*3:2*3]
    # test_pred, test_target = test_pred[:, :, 2*3:3*3], test_target[:, :, 2*3:3*3]
    test_pred, test_target = test_pred[:, :, 2], test_target[:, :, 2]
    # test_pred, test_target = test_pred[:, :, 5], test_target[:, :, 5]
    # test_pred, test_target = test_pred[:, :, 8], test_target[:, :, 8]
    # test_pred, test_target = test_pred[:, :, 11], test_target[:, :, 11]
    """draw PDF Fig"""
    ...
    '''Get the average peak error of acceleration in the training and test sets (output as character percentage)'''
    PeakError_max, PeakError_avg = f_max(y_true=test_target, y_pred=test_pred)
    MSE_t, RMSE_t, MAE_t, RMAE_t, r_t, R_list = time_performance(label=test_target, pred=test_pred)
...
class ModelConfig:
    def __init__(self, data_name):
        ...
        self.target_rate = 1  # Magnification factor for target value
        self.train_num = 10  # Number of training samples
        self.val_num = 0  # Number of validation samples
        self.test_num = 90  # Number of test samples
        self.train_batch = 10  # Batch size of training data
        self.dt = 0.02

        self.model_type1 = 'Hys1'
        ...
        self.trainLoader1, self.testLoader1 = self.get_data_loader(need_val=self.need_val)
        self.early_stopping1 = EarlyStopping(
            patience=1000, verbose=True, path=self.model_path1 + 'BModel.pth')  # early stopping
        self.hys = Hysteresis_Asym_MDOF_NewMark(dt=self.dt, device=self.device)

        self.loss_fn1 = Loss_PhyHysLSTM(Dif=self.finite_difference, hys=self.hys)
        # self.model_train1 = PhyLSTM3(n_hidden=self.N_hidden, Fin_DIF=self.finite_difference).to(self.device)
        self.model_train1 = torch.load('BModel.pth', map_location=torch.device('cuda')).to(self.device)
    ...
    def get_data_loader(self, need_val=True):
...
if __name__ == '__main__':
    ...
    dataname = "Asym_BL7dof_PhyLSTMwave_1500points"
